...
def database_builder():
    ...
    if os.path.isfile('anime_database.json'):
        ...
        urllib.request.urlretrieve('https://raw.githubusercontent.com/manami-project/anime-offline-database/master/anime-offline-database.json', 'anime_database.json')
        ...
        if not search_database(list_database, i) or not full_loaded:
            ...
            for im in torrent:
                ...
                if b'files' in im[b'info']:
                    ...
                    torrent_files.append(
                        {'name': im[b'info'][b'name'].decode("utf-8"), 'size': format_size_units(im[b'info'][b'length'])})
